...
#if defined(__aarch64__)
static inline uint64_t rdcycle_ticks(void) { uint64_t c; asm volatile("mrs %0, pmccntr_el0" : "=r"(c)); return c; }
#elif defined(__x86_64__)
...
/* ---- open() diagnostics (no logic change, just logs) ---- */
static void log_open_context(const char *path)
{
    ...
    if (stat(path, &st) == 0) {
        ...
        if (S_ISCHR(st.st_mode) || S_ISBLK(st.st_mode)) {
            ...
            LOG("[OPEN] rdev major:minor=%u:%u\n", maj, min);
        } else {
            LOG("[OPEN] note: not a char device (type=%c)\n", t);
        }
    } else {
        LOG("[OPEN] stat() failed (errno=%d: %s)\n", errno, strerror(errno));
    }
}
...
int main(int argc, char *argv[])
{
    ...
    if (fd < 0) {
        LOG("[OPEN] open(\"%s\", O_RDWR) failed: errno=%d (%s)\n",
            shm_path, errno, strerror(errno));
        fatal("open");
    }
    LOG("[OPEN] success: fd=%d\n", fd);
    LOG("[INFO] mmap...\n");
    struct shm_region *shm = mmap(NULL, sizeof(struct shm_region), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (shm == MAP_FAILED) fatal("mmap");
    LOG("[MMAP] addr=%p len=%zu prot=RW shared\n", (void*)shm, sizeof(struct shm_region));
    crypto_init();
    ...
    for (size_t idx = 0; idx < NUM_SIZES; ++idx) {
        ...
        // ========================================================================
        // LOOP 1: THROUGHPUT MEASUREMENT
        // ========================================================================
        __sync_synchronize();
        if (is_sender) {
            atomic_store_explicit(&shm->seq, 0, memory_order_release);
        //    LOG("[LOOP-SND] waiting for receiver to ack\n");
            while (atomic_load_explicit(&shm->ack, memory_order_acquire) != 0) cpu_relax();
        } else { // receiver
        //    LOG("[LOOP-RCV] waiting for sender to start\n");
            // *** FIX: Wait for sender's seq==0 (phase boundary) BEFORE resetting ack ***
            while (atomic_load_explicit(&shm->seq, memory_order_acquire) != 0) cpu_relax();
            atomic_store_explicit(&shm->ack, 0, memory_order_release);
        }
        __sync_synchronize();

        double mbps = 0.0;
        if (is_sender) {
            LOG("[LOOP-SND] starting throughput test size=%zu\n", msg_len);
            uint64_t throughput_t_start = now_ns();
            for (int i = 1; i <= ITERS; ++i) {
               // LOG("[LOOP-SND] iter %d\n", i);
                ...
                while (atomic_load_explicit(&shm->ack, memory_order_acquire) != (uint64_t)i) {
                    cpu_relax();
                 //   LOG("[LOOP-SND] waiting for ack %d\n", i);
                }
            }
            uint64_t throughput_t_end = now_ns();
            double sec = (double)(throughput_t_end - throughput_t_start) / 1e9;
            mbps = ((double)msg_len * ITERS) / (1024.0 * 1024.0 * sec);
        } else { // receiver
          //  LOG("[LOOP-RCV] starting throughput test size=%zu\n", msg_len);
            for (int i = 1; i <= ITERS; ++i) {
                while (atomic_load_explicit(&shm->seq, memory_order_acquire) != (uint64_t)i) {
                    cpu_relax();
                  //  LOG("[LOOP-RCV] waiting for seq %d\n", i);
                }
        ...
        double median_lat_us = 0.0;
        double median_cycles = 0.0;
        if (is_sender) {
            uint64_t latencies_ns[ITERS];
            uint64_t latencies_cycles[ITERS];

            for (int i = 0; i < ITERS; ++i) {
             //   LOG("[LOOP-SND] iter %d\n", i + 1);
                uint64_t iter_seq = (uint64_t)i + 1;

                // ** TIMER STARTED BEFORE MEMSET AS REQUESTED **
                uint64_t t_iter_start = now_ns();
                uint64_t c_iter_start = rdcycle_ticks();

#if CRYPTO_ENABLED
                write_header(tmp, session_id, iter_seq);
#endif
                memset(tmp + HDR_LEN, 0xA5, msg_len);

                crypto_encrypt(shm->buf, tmp, pt_len, session_id, iter_seq);
                atomic_store_explicit(&shm->seq, iter_seq, memory_order_release);

                while (atomic_load_explicit(&shm->ack, memory_order_acquire) != iter_seq) {
                    cpu_relax();
                //    LOG("[LOOP-SND] waiting for ack %llu\n", (unsigned long long)iter_seq);
                }

                uint64_t t_iter_end = now_ns();
                uint64_t c_iter_end = rdcycle_ticks();
                latencies_ns[i] = t_iter_end - t_iter_start;
                latencies_cycles[i] = c_iter_end - c_iter_start;
            }

            qsort(latencies_ns, ITERS, sizeof(uint64_t), compare_u64);
            qsort(latencies_cycles, ITERS, sizeof(uint64_t), compare_u64);

            uint64_t median_ns_val = (ITERS % 2 == 0) ?
                (latencies_ns[ITERS / 2 - 1] + latencies_ns[ITERS / 2]) / 2 :
                latencies_ns[ITERS / 2];
            uint64_t median_cycles_val = (ITERS % 2 == 0) ?
                (latencies_cycles[ITERS / 2 - 1] + latencies_cycles[ITERS / 2]) / 2 :
                latencies_cycles[ITERS / 2];

            median_lat_us = (double)median_ns_val / 1000.0;
            median_cycles = (double)median_cycles_val;

            // Print combined results
            printf("%zu,%.3f,%.2f,%.0f\n", msg_len, median_lat_us, mbps, median_cycles);
            fflush(stdout);
